...
struct BirdTextureCache { // texture cache implementation
  static std::unordered_map<BirdType, std::pair<std::unique_ptr<sf::Texture>, std::unique_ptr<sf::Texture>>> cache;
  ...
  static void createDefaultTexture(sf::Texture& texture) {
    ...
    for (int y = 0; y < 64; ++y) {
      for (int x = 0; x < 64; ++x) {
        ...
        if (distSq > 32.0f * 32.0f) continue;
        ...
        else if (dx > 8 && dx < 22 && dy > -18 && dy < -14 && dy < 0.5f * dx - 20) {
          std::copy_n(&eyebrow.r, 4, &pixels[idx]);
        }
      }
    }
